...
일단 이종혁은 1791보다 큰 수는 항상 빅뱅수 4개의 합으로 만들 수 있다는 것을 증명했다.
그리고 잠시 더 생각해보니 수가 충분히 크다면 항상 빅뱅수 3개의 합으로 만들 수 있다는 것도 증명했다.
...
이종혁은 1791보다 큰 수에 대해 답이 항상 4보다 작거나 같은 것을 알고 있고, 답이 아무리 커도 6을 넘을 수 없다는 것을 알고 있다.
또 답이 6인 것은 11과 26밖에 없다는 것을 알고 있다.
...
public class Q1229 {
    public static void main(String[] args) {
        // 왜 안풀리는지 의문 : https://www.acmicpc.net/problem/1229
        Scanner sc = new Scanner(System.in);
        int N = sc.nextInt();
        int[] bigbangNum = {66, 45, 28, 15, 6, 1};

        int count = 0;

        while(N != 0) {
            for (int num : bigbangNum) {
                if (N >= num) {
                    N -= num;
                    count++;
                    break;
