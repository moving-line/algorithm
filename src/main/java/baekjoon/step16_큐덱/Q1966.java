...
public class Q1966 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            int N = Integer.parseInt(info[i].split(" ")[0]); // order 하나의 사이즈
            int M = Integer.parseInt(info[i].split(" ")[1]); // 구할 값의 현재 위치

            Queue<Integer> q = new LinkedList<>();
            for (int j = 0; j < N; j++) q.add(Integer.parseInt(orders[i].split(" ")[j]));

            int answer = 0;

            while(!q.isEmpty()) {
                int max = 0;
                for (Integer num : q) if ( num >= max) max = num;

                if (q.element() < max) {
                    q.add(q.remove());
                    M = (M != 0) ? M - 1 : M + (q.size() - 1);
                }
                else {
                    q.remove();
                    answer++;
