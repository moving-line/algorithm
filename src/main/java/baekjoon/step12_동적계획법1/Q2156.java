...
public class Q2156 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= n; i++) quantity[i] = Integer.parseInt(br.readLine());
        ...
        for (int i = 2; i <= n; i++) {
            ...
            // 이번에 안뽑는다 : 전번에 뽑았던 안뽑았던 상관없다.

            dp[i][1] = Math.max(dp[i - 1][0] + quantity[i], quantity[i - 1] + dp[i - 2][0] + quantity[i]);
            // 이번에 뽑는다 : 전번에 안 뽑았거나, 전번에 뽑았으나 전전번엔 안뽑았다.
            // 전번에 안뽑은 경우 : dp[i-1][0] + quantity[i]
            // 전번에 뽑았으나 전전번엔 안뽑은 경우 : quantity[i - 1] + dp[i - 2][0] + quantity[i]
        }
