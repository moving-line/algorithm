...
public class 그리디_체육복 {
    public int solution(int n, int[] lost, int[] reserve) {
        int answer = n - lost.length; // 체육복 확정 학생 수

        // 자기 여분 잃어버린 학생 제거 편하게 리스트로 변환
        List<Integer> listL = new LinkedList<>();
        List<Integer> listR = new LinkedList<>();
        for (int i : lost) listL.add(i);
        for (int i : reserve) listR.add(i);
        ...
        for (int l : lost) {
            if (listR.contains(l)) {
                listL.remove((Integer) l);
                listR.remove((Integer) l);
                answer++;
            }
        }
        ...
        // 빌릴 수 있는 학생 탐색
        for (Integer l : listL) {
            if (listR.contains(l - 1)) {
                listR.remove((Integer) (l - 1));
                answer++;
            } else if (listR.contains(l + 1)) {
                listR.remove((Integer) (l + 1));
                answer++;
            }
        }
